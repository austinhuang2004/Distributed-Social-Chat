...
def direct_message(user_token, message, recipient):
    return json.dumps({'token': user_token, 'directmessage': {'entry': message, 'recipient': recipient, 'timestamp': f'{Post(message).get_time()}'}})


def requesting_directmessage(user_token, the_message):
    if the_message == 'new':
        return json.dumps({'token': user_token, 'directmessage': 'new'})
    elif the_message == 'all':
        return json.dumps({'token': user_token, 'directmessage': 'all'})
    else:
        return False


def parsing_directmessage(response):
    json_resp = json.loads(response.decode('utf-8').strip())
    if json_resp['response']['type'] != 'ok':
        return False
    return json_resp['response']['messages']
